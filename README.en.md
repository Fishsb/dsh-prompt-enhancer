...
- 🎛️ **5 optimization modes** — Basic (direct, fastest) / Lite (local rules) / Standard (rules + workspace & session retrieval) / Smart (LLM task-progress analysis + full retrieval) / **One-click Publish** (rough idea → complete implementable dev spec: 9-chapter structure, web retrieval, auto scenario routing (game/software), self-review verdict; iterative refinement)
