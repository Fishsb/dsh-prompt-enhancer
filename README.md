...
- 🎛️ **5 种优化模式**：基础（直发，最快）/ 轻量（本地规则）/ 标准（规则 + 工作区/会话检索）/ 智能（LLM 任务进度分析 + 全量检索）/ **一键发布**（输入粗略想法 → 网络检索同类项目结构 + 工作区参考，一键生成完整可实施的开发规格：九章结构、场景自动路由（游戏/软件）、方案自评判定，多轮补充可细化）
...
```sh
dsh plugin --profile web add github:Fishsb/dsh-prompt-enhancer#v2.8.0
```
...
> `#v2.8.0` 为版本锁定（可换成任意 [Release tag](https://github.com/Fishsb/dsh-prompt-enhancer/releases)），避免装到未发布的主分支。
...
从 [Releases 页面](https://github.com/Fishsb/dsh-prompt-enhancer/releases) 下载 `dsh-prompt-enhancer-<版本>.tgz`（如 `dsh-prompt-enhancer-2.8.0.tgz`），然后安装：

```sh
dsh plugin --profile web add ./dsh-prompt-enhancer-2.8.0.tgz
```
...
| 配置项 | 说明 |
|---|---|
| 优化模式 | 基础（默认，直发）/ 轻量 / 标准 / 智能 / 一键发布（生成完整开发规格：九章结构、网络检索、场景自动路由、方案自评判定，多轮补充细化）；切换即时生效并持久化 |
