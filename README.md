...
> `#v3.1.0` 为版本锁定（可换成任意 [Release tag](https://github.com/Fishsb/dsh-prompt-enhancer/releases)），避免装到未发布的主分支。
...
> 更新注意：tag 锁定安装（`github:...#v3.1.0`）时 `update` 不会跨 tag 升级——升级请用 `dsh plugin --profile web add github:Fishsb/dsh-prompt-enhancer#<新tag>`。
> 卸载不会清除浏览器端的插件配置（localStorage `dsh.enhance.config.v2`），重装后配置仍保留。

### 方式二：下载安装包离线安装

从 [Releases 页面](https://github.com/Fishsb/dsh-prompt-enhancer/releases) 下载 `dsh-prompt-enhancer-<版本>.tgz`（如 `dsh-prompt-enhancer-3.1.0.tgz`），然后安装：

```sh
dsh plugin --profile web add ./dsh-prompt-enhancer-3.1.0.tgz
```
