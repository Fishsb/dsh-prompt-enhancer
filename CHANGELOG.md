...
### Changed
- **连通性测试新增「基础模式默认模板预计耗时」（用户需求）**：测试连通性成功后，结果区追加实测 TTFT，并展示该模型历史 TTFT/tok/s 与「基础模式默认模板预计耗时」；预计耗时按当前输入长度动态估算输出 token（字符数 / 4，最低 200 token），历史数据实时读取 DSH 会话投影统计并按 provider/model 匹配；无匹配历史数据时隐藏预计耗时并提示。新增 `models/stats` RPC 与 U68 单测 — [PEN-004]

### Fixed
- **连通性测试历史统计卡「测试中…」（用户实测）**：`models/stats` 原先逐个 `readSession` 全量历史日志导致 30s+ 超时；改为 live 会话走内存、persisted 会话单次最多扫描 5 个并缓存 route/projection，客户端另加 8s 超时兜底，不再无限等待 — [PEN-004]
