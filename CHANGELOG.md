...
### Changed
- **桌面快捷方式图标 + 按钮文案优化（用户需求）**：① 快捷方式图标换 **DeepSeek 蓝色鲸鱼**——新增 `assets/deepseek.ico`（官方 favicon 下载入库），makeShortcut 时复制到 `EXECUTOR_ROOT/icons/deepseek.ico` → `.lnk` IconLocation（复制失败回退默认图标，不阻断）；② 端口重启确认态按钮文案「快捷」→「**桌面**」（i18n `updMakeShortcut`）；③ 按钮加悬停提示（title）「创建桌面快捷方式」（i18n 新增 `updShortcutTooltip` zh/en）；成功提示文案补充鲸鱼图标说明。全量测试 **152/152**；实机验证 .lnk 生成 + 图标文件复制正确（209KB 一致）— [PEN-002]
