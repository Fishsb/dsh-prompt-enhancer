...
### Changed
- **连通性测试新增「基础模式默认模板预计耗时」（用户需求）**：测试连通性成功后，结果区展示实测连通耗时，并展示该模型历史 TTFT/tok/s 与「基础模式默认模板预计耗时」；预计耗时按当前输入长度动态估算输出 token（字符数 / 4，最低 200 token），历史数据实时读取 DSH 会话投影统计，先按 provider/model 精确匹配、无命中时回退按模型名匹配；无匹配历史数据时明确提示。新增 `models/stats` RPC 与 U68 单测 — [PEN-004]
...
- **「暂无该模型历史统计」误报（用户实测）**：此前只按 provider+model 完全匹配，provider 路由别名不同（如 deepseek-official vs opencode）时同模型也判无数据；现改为精确匹配无命中时按模型名回退匹配 — [PEN-004]
