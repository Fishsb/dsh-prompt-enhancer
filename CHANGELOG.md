...
### Changed
- **移除开源许可证（用户指令）**：删除根目录 `LICENSE`（MIT）——同步清理：README.md/README.en.md 的 License 徽标与 License 章节、package.json `license` 字段与 `files` 白名单、更新器清单 `UPDATE_MANIFEST`/`UPDATER_MANIFEST`（host pure.js + client updater.js + 重新构建产物 plugin-host/plugin-client/client.cjs）与对应测试断言、AGENTS.md 发布白名单规则。**注意**：删除后仓库无明确许可证（默认保留所有权利），他人不可自由复用；更新器清单同步后旧版客户端校验 5 文件（原 6 去 LICENSE），发布时远端必须无 LICENSE 文件。129/129 — [PEN-002]
