...
function buildWebMenuCmdBody(opts) {
  ...
  const tail = 'if errorlevel 3 goto upd\r\n' +
  ...
  return Buffer.concat([
    ...
    G(ZH_GBK_LINES.opt1), CRLF,
    G(ZH_GBK_LINES.opt2), CRLF,
    G(ZH_GBK_LINES.opt3), CRLF,
    G(ZH_GBK_LINES.choice), CRLF,
...
harness.handle('update/makeShortcut', async (args) => {
    try {
      ...
      fs.writeFileSync(cmdPath, buildWebMenuCmdBody({ nodePath, target: staticTarget, svc, profile, locale }));
