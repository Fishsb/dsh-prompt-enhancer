 ...
 * updater-host.cjs 静态子命令，本壳只有 choice 分流）。用户规则 2026-08-22 最终版：
 * 一个图标、三功能全自动（[1] 启动 / [2] 端口修复 / [3] 一键更新）、choice /t 3 /d 1 默认 1。
 * 编码纪律：**纯 ASCII**——UTF-8 中文 + chcp 65001 会让 cmd 解析错位把行片段当命令执行
 * （2026-08-22 实测乱码根因）；node 子进程自己输出中文不受影响。
 */
function buildWebMenuCmdBody(opts) {
  ...
  return '@echo off\r\n' +
    'title DSH Web\r\n' +
    'echo === DSH Web ===\r\n' +
    'echo   [1] Start Web (ensure port online, open browser)\r\n' +
    'echo   [2] Repair port (auto-fix conflicts, fallback of [1])\r\n' +
    'echo   [3] One-click update (install staged, auto restart)\r\n' +
    'choice /c 123 /t 3 /d 1 /n /m "Select [1/2/3], default [1] in 3s: "\r\n' +
    'if errorlevel 3 goto upd\r\n' +
    'if errorlevel 2 goto rep\r\n' +
    esc(o.nodePath) + ' ' + esc(o.target) + ' --cli up ' + base + ' --open\r\n' +
    'pause\r\n' +
    'goto :eof\r\n' +
    ':rep\r\n' +
    esc(o.nodePath) + ' ' + esc(o.target) + ' --cli repair ' + base + '\r\n' +
    'pause\r\n' +
    'goto :eof\r\n' +
    ':upd\r\n' +
    esc(o.nodePath) + ' ' + esc(o.target) + ' --cli update ' + base + '\r\n' +
    'pause\r\n';
}
