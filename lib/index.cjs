...
harness.handle('update/makeShortcut', async (args) => {
    try {
      ...
      //    用 .ps1 文件方式避免复杂引号嵌套（路径均无单引号，直接拼入安全）。
      const desktop = path.join(process.env.USERPROFILE || process.env.HOME || '', 'Desktop');
      const lnkPath = path.join(desktop, '重启DSH服务.lnk');
      ...
      try {
        ...
        if (fs.existsSync(iconSrc)) {
          ...
          const iconPath = path.join(iconDir, 'deepseek.ico');
          fs.copyFileSync(iconSrc, iconPath);
          iconLine = "$s.IconLocation = '" + iconPath + "',0\r\n";
        }
      } catch { /* 图标复制失败 → 不设图标 */ }
      const psPath = path.join(cliDir, 'make-shortcut.ps1');
      const psBody = '$ws = New-Object -ComObject WScript.Shell\r\n' +
        "$s = $ws.CreateShortcut('" + lnkPath + "')\r\n" +
        "$s.TargetPath = '" + cmdExe + "'\r\n" +
        "$s.Arguments = '/c \"" + cmdPath + "\"'\r\n" +
        "$s.WorkingDirectory = '" + sys.EXECUTOR_ROOT + "'\r\n" +
        iconLine +
        "$s.Description = '重启 DSH 服务（脱离 Web 界面，CLI 展示步骤）'\r\n" +
        "$s.Save()\r\n" +
        "Write-Output 'OK'\r\n";
      // PowerShell 5.1 按 ANSI(GBK) 解析无 BOM 的 UTF-8 文件 → 中文引号错乱；加 UTF-8 BOM 修复
      fs.writeFileSync(psPath, '\uFEFF' + psBody, 'utf8');
      const r = spawnSync('powershell', ['-NoProfile', '-ExecutionPolicy', 'Bypass', '-File', psPath], { encoding: 'utf8', windowsHide: true, timeout: 30000 });
      try { fs.unlinkSync(psPath); } catch { /* ignore */ }
      if (r.status !== 0) {
        return { ok: false, code: 'SHORTCUT_FAILED', message: 'powershell create shortcut failed: ' + String(r.stderr || r.stdout || '').trim().slice(0, 200) };
      }
      if (!fs.existsSync(lnkPath)) {
        return { ok: false, code: 'SHORTCUT_MISSING', message: 'shortcut not created: ' + lnkPath };
      }
      return { ok: true, shortcutPath: lnkPath, cmdPath };
    } catch (e) {
