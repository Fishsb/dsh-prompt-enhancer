...
harness.handle('update/makeShortcut', async (args) => {
    try {
      ...
      // v3.3.x 批次二（方案 §5.1）：快捷方式更名「DSH Web 维护」；Desktop 分支整体删除——
      // 客户端重启走其设置页 RPC 自实例语义，桌面无需维护快捷方式。
      if (sys.isDesktop()) {
        return { ok: false, code: 'DESKTOP_UNSUPPORTED', message: 'DSH Desktop 无需维护快捷方式：请在设置页使用「重启客户端」（自实例重启）。' };
      }
      const lnkName = locale === 'en' ? 'DSH Web Maintenance.lnk' : 'DSH Web 维护.lnk';
      const nodePath = process.execPath;
      // v3.3.x 批次二（G4 载体决策定案）：不再生成一次性 restart-dsh.cjs 逻辑脚本（已删
      // buildCliRestartScript）。lnk 改指静态常驻子命令 <运行环境 lib>/updater-host.cjs
      // --cli maintain——updater-host.cjs 是随插件部署的静态文件，版本与插件天然一致，
      // 不会脚本丢失/逻辑漂移；本 RPC 只生成无逻辑的 .cmd 启动器（chcp 65001 + pause 回显壳）。
      ...
      // 1. 写无逻辑 .cmd 启动器 → 调 node 跑静态 updater-host.cjs 维护菜单
      const cmdPath = path.join(cliDir, 'dsh-maintain.cmd');
      const esc = (p) => '"' + String(p).replace(/"/g, '\\"') + '"';
      const cmdBody = '@echo off\r\n' +
        'chcp 65001 >nul\r\n' +
        'title DSH Web 维护\r\n' +
        esc(nodePath) + ' ' + esc(staticTarget) + ' --cli maintain --service ' + svc + ' --profile ' + profile + '\r\n' +
        'pause\r\n';
      ...
      // 按语言命名 + 删除旧名（重启DSH服务 / Restart DSH Service / 重启DSH / Restart DSH）避免重复
      ...
      // v3.3.x 批次二：旧「重启DSH」快捷方式被「DSH Web 维护」取代，创建时清理旧名
      ...
      // v3.3.x 批次二：Desktop 分支已整体删除（函数入口已拦截），web 恒打标。
