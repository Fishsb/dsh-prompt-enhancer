...
function spawnExecutorDirect(port) {
 ...
 */
function buildExecutorTaskXml(port, taskName, cmdPath) {
  ...
  return '<?xml version="1.0" encoding="UTF-16"?>' + CRLF +
    ...
    '<WorkingDirectory>' + xmlEscape(__dirname) + '</WorkingDirectory>' +
    '</Exec></Actions>' + CRLF +
    '</Task>' + CRLF;
}
...
function spawnExecutor(port) {
  ...
  const cmdContent = [
    ...
    '"' + process.execPath + '" "' + path.join(__dirname, 'updater-host.cjs') + '" --port ' + port + ' --dsh-bin "' + dshBin + '" --task "' + taskName + '" --cmd "' + cmdPath + '" >> "' + logPath + '" 2>&1',
  ].join(CRLF);
  try {
    fs.writeFileSync(cmdPath, cmdContent, 'utf8');
    const xml = buildExecutorTaskXml(port, taskName, cmdPath);
