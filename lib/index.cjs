...
harness.handle('update/serviceInstall', async (args) => {
  try {
    ...
    return { ok: true, message: '服务 ' + serviceName + ' 已安装（nssm ' + NSSM_VERSION + '）。请重启电脑或注销当前用户，让 nssm 以系统服务接管服务。' };
  } catch (e) {
 ...
 *
 * v3.2.1-h（用户指令·快捷方式与端口重启一样需要判断）：脚本开头先 `sc query` 判断
 * 当前是否 nssm 服务接管态（服务 RUNNING）——
 *   · 服务接管态 → `sc stop` → 等 STOPPED → **显式 `sc start`** → 等 3080 恢复。
 *     CLI 脚本由用户双击 cmd 运行（交互会话），**不在 nssm 的 Job 树内**，
 *     sc stop 树杀不会波及本脚本，可顺序执行完整重启，无需 schtasks 兜底。
 *   · 默认态（前台 DSH 托管 / 服务未运行）→ 原逻辑：杀旧前台 + 拉起新前台。
 * 杜绝「nssm 服务已接管 3080 时快捷方式又拉起一个前台 DSH 抢端口」的 EADDRINUSE 复现。
 */
function buildCliRestartScript({ execPath, fallbackBin, outLog, errLog, serviceName }) {
  const J = (v) => JSON.stringify(v);
  return [
    '// dsh-prompt-enhancer 独立 CLI 端口重启（v3.2.1-h：服务/默认双模式判断）',
    "const { spawnSync, spawn } = require('node:child_process');",
    "const fs = require('node:fs');",
    "const path = require('node:path');",
    ...
    // 服务接管态判断：sc query 输出含 STATE: N RUNNING 即 nssm 正在托管
    "const scq = spawnSync('sc', ['query', serviceName], { encoding: 'utf8', windowsHide: true });",
    "const scOut = String(scq.stdout || '') + String(scq.stderr || '');",
    'const svcRunning = /STATE\\s*:\\s*\\d+\\s+RUNNING/.test(scOut);',
    'if (svcRunning) {',
    "  console.log('检测到 nssm 服务 ' + serviceName + ' 正在接管，重启服务 ...');",
    ...
    "    if (r.status !== 0) console.log('注意：sc start 返回 ' + r.status + '（若端口恢复即正常）');",
    '    const poll = (n) => {',
    "      const rr = spawnSync('netstat', ['-ano'], { encoding: 'utf8', windowsHide: true });",
    '      const ok = /:3080\\s+\\S+\\s+LISTENING/.test(String(rr.stdout || \'\'));',
    "      if (ok) { console.log('✓ DSH 服务已恢复：http://127.0.0.1:3080'); setTimeout(() => process.exit(0), 500); return; }",
    "      if (n >= 40) { console.log('✗ 服务重启超时，请检查事件查看器或 ' + errLog); process.exit(1); }",
    ...
    "    if ((st && st[1] === 'STOPPED') || sw >= 15) { startSvc(); return; }",
...
// v3.2.1-g（用户最新指令·稳健服务重启）：nssm 服务的端口重启。
// 难点：DSH 以 nssm 服务运行时，本进程（host）是 nssm 的子进程、处于 nssm 的 Job 树内；
// 若在本进程内直接 `sc stop`，nssm 收到 stop 控制会树杀整个 Job（含本进程及其派生的任何
// 辅助脚本）→ 后续 `sc start` 永远来不及执行（历史多次失败的根因）。
// 解决：用 Task Scheduler 注册一次性任务 `sc start <svc>`（在 session 0 以 SYSTEM 运行，
// 完全在 nssm Job 树之外）→ 再 `sc stop` 当前服务。nssm 树杀只影响自身与服务 node，
// 调度任务在 ~12s 后触发 `sc start` 拉起全新 node 接管端口。任务执行后自删。
// 全程不依赖 nssm 自检（AppExit），确定性显式重启。
function scheduleServiceRestart(serviceName) {
  const tr = 'cmd /c "sc start ' + serviceName + ' & schtasks /delete /tn DSHPortRestart /f"';
  const when = schtasksOnceTime(12); // 12s 后触发：给 sc stop + 端口释放留足时间
  const create = spawnSync('schtasks', ['/create', '/tn', 'DSHPortRestart', '/tr', tr, '/sc', 'once', '/st', when, '/ru', 'SYSTEM', '/f'], { windowsHide: true, timeout: 15000 });
  if (create.status !== 0) {
    // 降级：无管理员/无 schtasks → 仅 sc stop（用户需手动在服务管理器启动）
    spawnSync('sc', ['stop', serviceName], { windowsHide: true, timeout: 15000 });
    return;
  }
  // 任务已注册（12s 后自动 sc start）；立即 sc stop 当前服务——nssm 树杀自身，不影响调度任务
  spawnSync('sc', ['stop', serviceName], { windowsHide: true, timeout: 15000 });
}
...
harness.handle('update/portRestart', async (args) => {
  try {
    ...
    // v3.2.1-g（用户最新指令·端口重启加判断能力）：根据「当前谁在托管端口」决定重启方式——
    //   · nssm 服务接管态（服务 EXISTS 且 RUNNING）→ 走服务重启（见 scheduleServiceRestart：
    //     Task Scheduler 调度 sc start 脱离 nssm Job 树杀 + sc stop 当前服务）
    //   · 默认态（前台 DSH 托管 / 服务未运行）→ 走独立 detached 脚本（杀旧前台 + 拉起新前台）
    // 不在端口重启里做「强制接管」——安装后由系统重启/注销接管（见 update/serviceInstall）。
    const backend = platformService.backendFor(process.platform);
    ...
    const serviceActive = serviceExists && backend ? !backend.isStopped(serviceName, envForProbe()) : false;

    if (serviceActive) {
      // 服务模式：调度 sc start（Task Scheduler，脱离 nssm Job 树）+ sc stop 当前服务
      scheduleServiceRestart(serviceName);
      return {
        ok: true,
        message: '端口重启已触发（服务模式·nssm 接管，sc 重启服务 ' + serviceName + '）',
        serviceMode: true,
      };
    }
