...
/**
 * 无逻辑 .cmd 启动器体（makeShortcut 与契约测试共用；G4：逻辑全部在 updater-host.cjs 静态子命令）。
 */
function buildLauncherCmdBody(opts) {
  const o = opts || {};
  const esc = (p) => '"' + String(p).replace(/"/g, '\\"') + '"';
  return '@echo off\r\n' +
    'chcp 65001 >nul\r\n' +
    'title ' + (o.title || 'DSH Web') + '\r\n' +
    esc(o.nodePath) + ' ' + esc(o.target) + ' ' + o.cliArgs + '\r\n' +
    'pause\r\n';
}
...
harness.handle('update/makeShortcut', async (args) => {
    try {
      ...
      // 旧名（重启DSH服务 / Restart DSH Service / 重启DSH / Restart DSH）删除避免重复。
      const locale = args && typeof args.locale === 'string' && /^(zh|en)$/.test(args.locale) ? args.locale : 'zh';
      // v3.3.x 批次二（方案 §5.1）：Desktop 分支整体删除——客户端重启走其设置页 RPC 自实例语义。
      if (sys.isDesktop()) {
        return { ok: false, code: 'DESKTOP_UNSUPPORTED', message: 'DSH Desktop 无需维护快捷方式：请在设置页使用「重启客户端」（自实例重启）。' };
      }
      const nodePath = process.execPath;
      // v3.3.x 批次二（G4 载体决策定案）：不再生成一次性 restart-dsh.cjs 逻辑脚本。lnk 指静态
      // 常驻子命令 <运行环境 lib>/updater-host.cjs——随插件部署、版本天然一致，不会丢失/漂移；
      // 本 RPC 只生成无逻辑 .cmd 启动器。2026-08-22（用户需求）：双入口——
      //   「DSH Web 维护」= --cli maintain 全功能菜单（保留 RunAs 提权，菜单多项需管理员）；
      //   「DSH Web 启动」= --cli up --open 一键拉起任何状态确保在线（健康即报/僵尸处置/
      //     服务路径/前台冷启兜底）——日常双击入口，刻意**不打** RunAs 标志（免每次 UAC，
      //     冷启路径无需管理员；服务路径失败自动降级前台）。
      ...
      const launchers = [
        {
          key: 'maintain',
          lnkName: locale === 'en' ? 'DSH Web Maintenance.lnk' : 'DSH Web 维护.lnk',
          cmdName: 'dsh-maintain.cmd',
          title: locale === 'en' ? 'DSH Web Maintenance' : 'DSH Web 维护',
          cliArgs: '--cli maintain --service ' + svc + ' --profile ' + profile,
          runAs: true, // 菜单多项需管理员
        },
        {
          key: 'up',
          lnkName: locale === 'en' ? 'DSH Web Start.lnk' : 'DSH Web 启动.lnk',
          cmdName: 'dsh-web-up.cmd',
          title: locale === 'en' ? 'DSH Web Start' : 'DSH Web 启动',
          cliArgs: '--cli up --service ' + svc + ' --profile ' + profile + ' --open',
          runAs: false, // 日常双击免 UAC；权限不足自动降级前台冷启
        },
      ];
      // 1. 写无逻辑 .cmd 启动器
      for (const L of launchers) {
        const p = path.join(cliDir, L.cmdName);
        fs.writeFileSync(p, buildLauncherCmdBody({ nodePath, target: staticTarget, title: L.title, cliArgs: L.cliArgs }), 'utf8');
        L.cmdPath = p;
      }
      ...
      // v3.3.x 批次二：旧「重启DSH」快捷方式被取代，创建时清理旧名
      ...
      const results = [];
      for (const L of launchers) {
        const lnkPath = path.join(desktop, L.lnkName);
        ...
        if (!winResult.ok) {
          return { ok: false, code: winResult.code || 'SHORTCUT_FAIL', message: (L.lnkName + ': ' + (winResult.message || 'shortcut creation failed')) };
        }
        if (!fs.existsSync(lnkPath)) {
          return { ok: false, code: 'SHORTCUT_MISSING', message: 'shortcut not created: ' + lnkPath };
        }
        // RunAs 标志仅维护入口需要（双击即 UAC）；启动入口免提权（冷启路径无需管理员）
        const runAsApplied = L.runAs ? markLnkRunAs(lnkPath) : false;
        ...
        results.push({ key: L.key, shortcutPath: lnkPath, cmdPath: L.cmdPath, iconApplied, lnkSize, runAsApplied });
      }
      return { ok: true, shortcuts: results, method: 'cscript',
        shortcutPath: results[0].shortcutPath, cmdPath: results[0].cmdPath,
        iconApplied: results.every((r) => r.iconApplied), runAsApplied: results[0].runAsApplied };
    } catch (e) {
...
module.exports = {
  ...
  // 批次二（用户需求·一键拉起）：.cmd 启动器体生成器导出——快捷方式契约单测用
  buildLauncherCmdBody,
  apply(ctx) {
