    ...
    if (run.status === 0) {
      ...
      setTimeout(() => {
        try {
          ...
          if (d.status === 0) {
            console.log('[dsh-prompt-enhancer] delayed sweep: exec task deleted (' + taskName + ')');
          } else {
            console.log('[dsh-prompt-enhancer] delayed sweep delete FAILED exit=' + d.status +
              ' stderr=' + String(d.stderr || '').trim().slice(0, 150) + ' — janitor menu will clean up');
          }
        } catch (e) {
          console.log('[dsh-prompt-enhancer] delayed sweep error: ' + String(e && e.message || e));
        }
      }, 60000).unref();
      return null;
    }
...
function autoGateDecision(isAuto, opts) {
  ...
  if (ks) {
    console.log('[enhance] portRestart auto REJECTED by kill-switch (' + ks + ')');
  ...
  if (maintainLib.shouldGateAuto(st || {}, Date.now())) {
    ...
    console.log('[enhance] portRestart auto REJECTED by backoff (wait=' + waitS + 's dayCount=' + (Number(s.dayCount) || 0) + '/' + maintainLib.AUTO_DAILY_LIMIT + ')');
...
function scheduleServiceRestart(serviceName, seam) {
  ...
  if (nowMs - prevScheduleAt < windowMs || (() => {
    ...
    console.log('[enhance] scheduleServiceRestart debounced (pending window)');
  ...
  try {
    ...
    if (prev.restartScript && Number.isInteger(prev.pendingRestartAt) &&
      ...
      if (taskGone) {
        try { fs.unlinkSync(prev.restartScript); console.log('[enhance] cleaned expired restart script: ' + path.basename(prev.restartScript)); } catch { /* 可能已被自删 */ }
      }
    }
  } catch { /* 清理失败不阻断主流程 */ }
  ...
  try {
    ...
    console.log('[enhance] debounce record write failed (fail-open): ' + String(e && e.message || e));
  }
  if (run.status !== 0) {
