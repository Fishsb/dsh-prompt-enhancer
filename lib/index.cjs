...
function scheduleServiceRestart(serviceName) {
  ...
  // 任务（powershell，SYSTEM 运行、脱离 nssm Job 树）：3s → sc stop（杀旧服务）→
  // 2s → 清 3080 残留占用（服务存在未运行 + 前台占场景）→ 2s → sc start → 自删。
  // 注：必须 sc.exe（PowerShell 的 sc 是 Set-Content 别名）；$c 在 JS 字符串无转义问题。
  const ps = 'Start-Sleep 3; sc.exe stop ' + serviceName + ' | Out-Null; Start-Sleep 2; '
    + '$c=Get-NetTCPConnection -LocalPort 3080 -State Listen -ErrorAction SilentlyContinue; '
    + 'if($c){Stop-Process -Id $c.OwningProcess -Force -ErrorAction SilentlyContinue}; '
    + 'Start-Sleep 2; sc.exe start ' + serviceName + ' | Out-Null; '
    + 'schtasks /delete /tn ' + taskName + ' /f';
  const tr = 'powershell -NoProfile -WindowStyle Hidden -Command "' + ps + '"';
  ...
  if (create.status !== 0) {
    // 降级：无管理员/无 schtasks → 仅 sc stop（host 可能被杀；client 侧 fire-and-forget 轮询兜底）
