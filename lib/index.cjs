...
// 20s 内未确认仍兜底写一次（非服务化 spawn 降级依赖索引的 argv/execPath，必须保证可用）。
{
  ...
  // 探测：3080 是否由本进程（PID）监听
  const isListening3080 = () => {
    try {
      const r = spawnSync('netstat', ['-ano'], { encoding: 'utf8', timeout: 3000 });
      const pid = String(process.pid);
      const lines = String(r.stdout || '').split(/\r?\n/);
      return lines.some((l) => l.indexOf(':3080') !== -1 && l.indexOf('LISTENING') !== -1 && l.indexOf(pid) !== -1);
    } catch (e) { return false; }
  };
  let tries = 0;
  const idxTimer = setInterval(() => {
    tries++;
    if (isListening3080() || tries >= 40) { // 确认监听 或 20s 兜底
      writeIndex();
