...
function buildPortRestartScript({ execPath, argv, cwd, oldPid, outLog, errLog }) {
  ...
  return [
    ...
    '  }, 1000);',
    '}, 1000);',
    '// v3.2.1-k（清理残留·待办）：脚本执行完毕后自删（延迟 4s，避免与 fd 关闭竞态）',
    "setTimeout(() => { try { fs.unlinkSync(__filename); } catch (e) {} }, 4000);",
  ].join('\n');
}
...
function buildCliRestartScript({ execPath, fallbackBin, outLog, errLog, serviceName }) {
  ...
  return [
    ...
    '  // v3.2.1-k（用户确认·权限）：sc stop/start 需要管理员权限——非管理员直接明确失败提示',
    '  // （lnk 已设 RunAs 双击即提权；此检测防绕过 lnk 直接运行脚本的场景）',
    '  let isAdmin = false;',
    "  try { isAdmin = spawnSync('net', ['session'], { windowsHide: true, stdio: 'ignore' }).status === 0; } catch (e) { isAdmin = false; }",
    '  if (!isAdmin) {',
    "    console.log('✗ 执行失败：服务模式需要管理员权限，请以管理员身份运行本快捷方式（右键 → 以管理员身份运行）');",
    '    process.exit(1);',
    '  }',
