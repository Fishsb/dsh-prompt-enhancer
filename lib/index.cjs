...
const ZH_GBK_LINES = {
  ...
  failMsg: '6563686f205b785d20c6f4b6afcaa7b0dca1aaa1aad2d1b7b5bbd8b2cbb5a5a3acbfc9cad4205b325d20b6cbbfdad0deb8b4a1a3',
  updFailMsg: '5b785d20b8fcd0c2caa7b0dca3a1d2d1b7b5bbd8b2cbb5a5a1a3',
};

/**
 * v4.13 批次B（P0-2）·自愈守卫探针脚本（生成物，落 executor/cli/dsh-heal-guard.cjs）。
 * cmd 壳无 JSON/比较能力 → 生成时内联 node 探针前置；壳保持 ASCII 短链。
 * 独立性纪律：探针自算基线 max(账本版本, 运行环境 package.json 现读版本)——不 require
 * 主模块、不依赖宿主传版本（单空用非空者、双空=盲区放行）；缓存版本不可读=拒绝+留痕
 * （B-3：宁可显性失败不可静默降级）。紧急旋钮 env DSH_PE_HEAL_OVERRIDE=1 全放行并留痕。
 * B-5 漂移锚：本脚本文本由单测抽出 cmpVersions 与 maintain-lib.compareSemver 对向量断言一致。
 */
function buildHealGuardScript() {
  return [
    '// dsh-heal-guard.cjs - generated by dsh-prompt-enhancer (batch B v4.13). Do not edit.',
    '// Usage:',
    '//   node dsh-heal-guard.cjs <cacheTgz> <runtimeRoot>   decide: exit 0 allow / 1 refuse',
    '//   node dsh-heal-guard.cjs --mark <runtimeRoot>       after restore: record deploy ledger',
    "// Baseline = max(ledger version, runtime package.json version); both empty = blind spot, allow.",
    '// Cache version unreadable = refuse (suspicious package). Env DSH_PE_HEAL_OVERRIDE=1 = bypass.',
    "'use strict';",
    'const fs = require("fs");',
    'const path = require("path");',
    'const zlib = require("zlib");',
    'function homeDir() { return process.env.DSH_HOME || path.join(process.env.USERPROFILE || process.env.HOME || "", ".dsh"); }',
    'function readJsonNoBom(p) { try { return JSON.parse(fs.readFileSync(p, "utf8").replace(/^\\uFEFF/, "")); } catch (e) { return null; } }',
    'function cmpVersions(a, b) {',
    '  function p3(s) { const m = /^v?(\\d+)\\.(\\d+)\\.(\\d+)/.exec(String(s || "").trim()); return m ? [Number(m[1]), Number(m[2]), Number(m[3])] : null; }',
    '  const pa = p3(a), pb = p3(b);',
    '  if (!pa || !pb) return null;',
    '  for (let i = 0; i < 3; i++) { if (pa[i] !== pb[i]) return pa[i] < pb[i] ? -1 : 1; }',
    '  return 0;',
    '}',
    'function peekTarballVersion(p) {',
    '  try {',
    '    const tar = zlib.gunzipSync(fs.readFileSync(p)).toString("latin1");',
    '    const m = /"name"\\s*:\\s*"dsh-prompt-enhancer"[\\s\\S]{0,600}?"version"\\s*:\\s*"([\\w.\\-+]+)"/.exec(tar);',
    '    return m ? m[1] : "";',
    '  } catch (e) { return ""; }',
    '}',
    'function refuseLog(msg) {',
    '  try { fs.appendFileSync(path.join(__dirname, "heal-refused.log"), "[" + new Date().toISOString() + "] " + msg + "\\n", "utf8"); } catch (e) {}',
    '}',
    'function baselineOf(runtimeRoot, profile) {',
    '  let lv = "";',
    '  try {',
    '    const led = readJsonNoBom(path.join(homeDir(), "dsh-prompt-enhancer.deploy.json"));',
    '    const e = led && led.profiles && led.profiles[profile];',
    '    if (e && typeof e.version === "string") lv = e.version;',
    '  } catch (e) {}',
    '  let rv = "";',
    '  try {',
    '    const pj = runtimeRoot ? readJsonNoBom(path.join(runtimeRoot, "package.json")) : null;',
    '    if (pj && typeof pj.version === "string") rv = pj.version;',
    '  } catch (e) {}',
    '  let base = "";',
    '  for (const v of [lv, rv]) { if (v && (!base || cmpVersions(v, base) > 0)) base = v; }',
    '  return { ledgerVersion: lv, runtimeVersion: rv, baseline: base };',
    '}',
    'const args = process.argv.slice(2);',
    'const PROFILE = "web"; // heal flow restores the web profile today (TGZDIR hardcodes web)',
    'if (args[0] === "--mark") {',
    '  const rtRoot = args[1] || "";',
    '  const pj = readJsonNoBom(path.join(rtRoot, "package.json"));',
    '  const ver = pj && typeof pj.version === "string" ? pj.version : "";',
    '  const ledPath = path.join(homeDir(), "dsh-prompt-enhancer.deploy.json");',
    '  const cur = readJsonNoBom(ledPath) || {};',
    '  const next = Object.assign({}, cur, { schema: cur.schema || 1, profiles: Object.assign({}, cur.profiles) });',
    '  next.profiles[PROFILE] = { version: ver, source: "heal", ts: Date.now() };',
    '  const tmp = ledPath + ".tmp";',
    '  fs.mkdirSync(path.dirname(ledPath), { recursive: true });',
    '  fs.writeFileSync(tmp, JSON.stringify(next, null, 2), "utf8");',
    '  fs.renameSync(tmp, ledPath);',
    '  console.log("[guard] deploy ledger marked: " + PROFILE + " @ " + (ver || "?") + " (source=heal)");',
    '  process.exit(0);',
    '}',
    'const cacheTgz = args[0] || "";',
    'const rtRoot = args[1] || "";',
    'if (String(process.env.DSH_PE_HEAL_OVERRIDE || "") === "1") {',
    '  console.log("[guard] DSH_PE_HEAL_OVERRIDE=1 - heal guard bypassed by operator.");',
    '  refuseLog("bypass DSH_PE_HEAL_OVERRIDE=1 cache=" + cacheTgz);',
    '  process.exit(0);',
    '}',
    'if (!cacheTgz || !fs.existsSync(cacheTgz)) process.exit(0); // nothing to restore - shell handles its own no-package branch',
    'const cacheVer = peekTarballVersion(cacheTgz);',
    'const b = baselineOf(rtRoot, PROFILE);',
    'if (!cacheVer) {',
    '  refuseLog("refuse CACHE_VERSION_UNREADABLE cache=" + cacheTgz + " ledger=" + b.ledgerVersion + " runtime=" + b.runtimeVersion);',
    '  console.log("[guard] REFUSED: cache package version unreadable (BOM or corrupt tgz). Restore blocked - see cli/heal-refused.log");',
    '  process.exit(1);',
    '}',
    'if (!b.baseline) {',
    '  console.log("[guard] blind spot (no ledger/runtime version) - allow first-install style restore. cache=" + cacheVer);',
    '  process.exit(0);',
    '}',
    'const cmp = cmpVersions(cacheVer, b.baseline);',
    'if (cmp === null || cmp < 0) {',
    '  refuseLog("refuse DOWNGRADE cache=" + cacheVer + " baseline=" + b.baseline + " (ledger=" + b.ledgerVersion + " runtime=" + b.runtimeVersion + ")");',
    '  console.log("[guard] REFUSED: cache " + cacheVer + " is older than deployed baseline " + b.baseline + ". Stale-cache downgrade blocked - see cli/heal-refused.log");',
    '  process.exit(1);',
    '}',
    'console.log("[guard] allow: cache " + cacheVer + " >= baseline " + b.baseline);',
    'process.exit(0);',
  ].join('\n') + '\n';
}
...
function buildWebMenuCmdBody(opts) {
  ...
  // 失败提示行：同上双编码
  const failMsg = isZh ? G(ZH_GBK_LINES.failMsg) : A('echo [x] Start failed - back to menu, try [2] Repair port.');
  // v4.8：[3] 更新失败提示（同上双编码；成功路径 pause 后直接关窗——服务已在后台重启完毕）
  const updFailMsg = isZh ? G(ZH_GBK_LINES.updFailMsg) : A('echo [x] Update failed - back to menu.');
  ...
  return Buffer.concat([
    ...
    ...(heal ? [
      ...
      A('call :minfix\r\n'),
      A('if exist ' + esc(o.target) + ' goto menu\r\n'),
      A('echo [x] Runtime missing and auto-repair failed. Web core rescue attempted - check http://127.0.0.1:3080\r\n'),
    ...
    ...(heal ? [
      ...
      A('if defined DSH_HOME set "TGZDIR=%DSH_HOME%\\profiles\\web\\plugins\\dsh-prompt-enhancer-tgz"\r\n'),
      ...
      A('if not defined NEWEST (echo [x] no local plugin package found for self-repair.& endlocal& exit /b)\r\n'),
      // v4.13 批次B（P0-2）：heal 前置版本守卫——探针脚本自算基线 max(账本, 运行环境现读)，
      // 缓存旧于基线/版本不可读 → 拒绝恢复并留痕 cli/heal-refused.log，跳转 :minfix 裸救援。
      // 守卫脚本文件缺失按无守卫放行（fail-open：helper 缺失不得复刻「三功能全灭」失败态；
      // 守卫自身的裁决则 fail-closed）。DSH_PE_HEAL_OVERRIDE=1 时探针内部直接放行。
      A('set "GUARD=%~dp0dsh-heal-guard.cjs"\r\n'),
      A('if not exist "%GUARD%" echo [i] heal guard script missing - proceeding without version check.\r\n'),
      A('if exist "%GUARD%" ' + esc(o.nodePath) + ' "%GUARD%" "%NEWEST%" "' + rtRoot + '"\r\n'),
      A('if errorlevel 1 (echo [x] Heal REFUSED by version guard: stale cache would downgrade the deployed version. See cli/heal-refused.log& endlocal& goto minfix)\r\n'),
      A('echo [i] Restoring runtime from %NEWEST% ...\r\n'),
      A('tar -xzf "%NEWEST%" -C "%HEAL%" || (endlocal& exit /b)\r\n'),
      A('xcopy "%HEAL%\\package\\*" "' + rtRoot + '\\" /e /y /i /q >nul\r\n'),
      // v4.13 批次B（写入点④）：恢复成功即回写账本（source='heal'，版本读恢复后运行环境 package.json）
      A('if exist "%GUARD%" ' + esc(o.nodePath) + ' "%GUARD%" --mark "' + rtRoot + '"\r\n'),
      A('rd /s /q "%HEAL%" 2>nul\r\n'),
      A('endlocal\r\n'),
      A('exit /b\r\n'),
      // v4.9.1（审查 M2 修正）：裸救援权限边界如实化——非提权下 taskkill(SYSTEM 持口)/sc start 可能被拒：
      // 预检提示 + 失败按权限分支给指引；移除 sc config start= auto 的持久副作用。
      A('\r\n:minfix\r\n'),
      A('rem v4.9.1 minimal port rescue - pure system commands, no plugin runtime needed\r\n'),
      A('setlocal enabledelayedexpansion\r\n'),
      A('set "ADMIN=1"\r\n'),
      A('net session >nul 2>&1 || set "ADMIN=0"\r\n'),
      A('if "%ADMIN%"=="0" echo [i] Non-admin session: rescue may be limited if the port holder is SYSTEM.\r\n'),
      ...
      A('echo [x] Port 3080 still down after 30s.\r\n'),
      A('if "%ADMIN%"=="0" echo     Hint: run this shortcut as administrator and choose [2] again.\r\n'),
      A('echo     Check nssm/eventlog for details.\r\n'),
      A('exit /b\r\n'),
      A(':mdone\r\n'),
...
harness.handle('update/makeShortcut', async (args) => {
    try {
      ...
      // v4.13 批次B（P0-2）：同步落自愈守卫探针脚本（壳 :heal 前置判定用；幂等覆盖，版本随插件走）
      try { fs.writeFileSync(path.join(cliDir, 'dsh-heal-guard.cjs'), buildHealGuardScript(), 'utf8'); } catch { /* 写失败=壳按无守卫放行并提示 */ }
      ...
      const winResult = makeShortcutWin({
        lnkPath,
        target: cmdExe,
        args: '/c "' + cmdSafeValue(cmdPath, true) + '"',
...
/** 解析稳定的 dsh bin.js：优先全局安装（dsh-install，路径稳定），回退当前进程 argv[1]
 *（npx 缓存路径含日期目录，可能被清理——Desktop 分支重启须指向不会被清掉的路径）。 */
...
// 不在端口重启里做「强制接管」——服务化安装入口已于 2026-09-12 按用户指令移除（插件不再代装 nssm）。
/** 服务存在性提示（handler 内联用，避免重复 !! 链）。 */
function serviceExistsHint(svcInfo) { return !!(svcInfo && svcInfo.exists); }

/**
 * 2026-09-12（D1 修复）：DSH err 日志尾部（端口重启失败根因）——实现单一事实源在执行器模块
 * lib/updater-host.cjs（含脱敏），此处惰性 require（不在 host 启动期加载执行器模块）；任何失败静默空串。
 */
...
// v4.12 批次A（P0-1）：自动链总开关读取——env DSH_PE_AUTO_UPDATE=0 ∥ config
// update.autoUpdate===false → 返回来源标识（'env'|'config'）；都未设返回 ''（自动链放行）。
// 手动操作不受限；读法复用 voice/modelDownload 读 download.proxy 的容错先例（lib/index.cjs:526-538）。
function readAutoUpdateKillSwitch(opts) {
  const o = opts || {};
  const envVal = o.envValue !== undefined ? o.envValue : process.env.DSH_PE_AUTO_UPDATE;
  if (String(envVal).trim() === '0') return 'env';
  const cf = o.configFile !== undefined ? o.configFile : CONFIG_FILE;
  try {
    if (cf && fs.existsSync(cf)) {
      const cfg = JSON.parse(fs.readFileSync(cf, 'utf8'));
      if (cfg && cfg.update && cfg.update.autoUpdate === false) return 'config';
    }
  } catch { /* 配置读失败=开关未设 */ }
  return '';
}

/** 批次A：update 域私有状态文件读取（损坏/缺失按空状态——闸门 fail-open 放行）。 */
function readUpdateStateSafe(opts) {
  const o = opts || {};
  try {
    const p = (o && o.stateFileOverride) || sys.updateStateFile();
...
/**
 * 批次A：update 域私有状态文件原子写（tmp+rename，config/set 先例）。
 * 中途失败只留 .tmp 孤儿、目标文件保持上一份完整 JSON——绝不产生半截状态。
 */
function writeUpdateStateSafe(next, opts) {
  const o = opts || {};
  const p = (o && o.stateFileOverride) || sys.updateStateFile();
...
/**
 * v4.12 批次A（P0-1）：自动链闸门决策（纯逻辑抽出便于单测；handler 仅消费其结果）。
 * isAuto=false 恒放行（手动豁免）；auto 调用依次查 kill-switch（→AUTO_DISABLED）与退避闸
 * （→BACKOFF_WAITING，message 带剩余秒数）。opts 全部可注入：envValue/configFile/state。
 */
function autoGateDecision(isAuto, opts) {
  if (!isAuto) return null;
  const o = opts || {};
  const ks = readAutoUpdateKillSwitch({ envValue: o.envValue, configFile: o.configFile });
  if (ks) {
    logT('[enhance] portRestart auto REJECTED by kill-switch (' + ks + ')');
    return { code: 'AUTO_DISABLED', message: '自动更新链已被关闭（' + (ks === 'env' ? '环境变量 DSH_PE_AUTO_UPDATE=0' : '配置 update.autoUpdate=false') + '），本次自愈调度跳过；如需重启请手动点击「端口重启」' };
  }
  let st = o.state;
  if (!st) {
    try { st = readUpdateStateSafe({ stateFileOverride: o.stateFileOverride }); } catch { st = {}; }
  }
  if (maintainLib.shouldGateAuto(st || {}, Date.now())) {
    const s = st || {};
    const waitS = Math.max(0, Math.ceil(((Number(s.nextRetryAt) || 0) - Date.now()) / 1000));
    logT('[enhance] portRestart auto REJECTED by backoff (wait=' + waitS + 's dayCount=' + (Number(s.dayCount) || 0) + '/' + maintainLib.AUTO_DAILY_LIMIT + ')');
    return { code: 'BACKOFF_WAITING', message: '更新下载失败退避中，' + waitS + ' 秒后才允许自动重试（当日第 ' + (Number(s.dayCount) || 0) + '/' + maintainLib.AUTO_DAILY_LIMIT + ' 次）；如需立即处理请手动点击「端口重启」' };
  }
  return null;
}
 ...
 */
// v4.12 批次A（P2-6）防抖窗口：≥120s 兜底窗 + 单轮重启实测 40-70s，取 5 分钟；参数化便于校准。
const RESTART_DEBOUNCE_MS = 5 * 60 * 1000;
let lastScheduleAt = 0; // 双闸①：进程内时间戳（零 IO）
function scheduleServiceRestart(serviceName, seam) {
  const s2 = seam || {};
  const spawnImpl = s2.spawnSyncImpl || spawnSync;
  const windowMs = Number.isInteger(s2.windowMs) && s2.windowMs >= 0 ? s2.windowMs : RESTART_DEBOUNCE_MS;
  const taskName = 'DSHPortRestart';
  // ---- P2-6 防抖双闸（先判后写：跳过路径绝不生成新脚本/新任务）----
  // （lastScheduleAt 经 seam 可注入覆盖——单测显式控制双闸①状态，生产路径不受影响）
  const nowMs = Date.now();
  const prevScheduleAt = Number.isInteger(s2.lastScheduleAt) ? s2.lastScheduleAt : lastScheduleAt;
  if (nowMs - prevScheduleAt < windowMs || (() => {
    // 双闸②：跨进程文件闸（覆盖 host 重启后短时间内再次调用的情形）；读失败按未阻塞
    try { return maintainLib.isDebounceBlocked(readUpdateStateSafe({ stateFileOverride: s2.stateFileOverride }), nowMs, windowMs); } catch { return false; }
  })()) {
    logT('[enhance] scheduleServiceRestart debounced (pending window)');
    return { ok: true, debounced: true, message: '已有待触发的重启任务，本次跳过' };
  }
  // ---- 安全清理路径①（评审 P2-6：禁止「命中即清理」——只在放行写入点清理已过期旧脚本）：
  // 窗口过期后，上次的 DSHPortRestart 要么已触发自删、要么 /run 失败留守但触发点已过；
  // 可选 schtasks /query 复核不在后清掉记录中的旧脚本，再写新记录。
  try {
    const prev = readUpdateStateSafe({ stateFileOverride: s2.stateFileOverride });
    if (prev.restartScript && Number.isInteger(prev.pendingRestartAt) &&
        (nowMs - prev.pendingRestartAt) >= windowMs) {
      let taskGone = true;
      if (!s2.spawnSyncImpl) {
        const qr = spawnSync('schtasks', ['/Query', '/TN', 'DSHPortRestart'], { windowsHide: true, timeout: 15000 });
        taskGone = qr.status !== 0;
      }
      if (taskGone) {
        try { fs.unlinkSync(prev.restartScript); logT('[enhance] cleaned expired restart script: ' + path.basename(prev.restartScript)); } catch { /* 可能已被自删 */ }
      }
    }
  } catch { /* 清理失败不阻断主流程 */ }
  ...
  const create = spawnImpl('schtasks', ['/create', '/tn', taskName, '/tr', tr, '/sc', 'once', '/st', when, '/ru', 'SYSTEM', '/f'], { windowsHide: true, timeout: 15000 });
  logT('[enhance] scheduleServiceRestart create status=' + create.status + ' stderr=' + String(create.stderr || '').slice(0, 200));
  if (create.status !== 0) {
  ...
  const run = spawnImpl('schtasks', ['/run', '/tn', taskName], { windowsHide: true, timeout: 15000 });
  logT('[enhance] scheduleServiceRestart run status=' + run.status + ' stderr=' + String(run.stderr || '').slice(0, 200));
  // v4.12 批次A（P2-6·评审修正②）：写防抖记录 {pendingRestartAt=120s 兜底时刻, restartScript}。
  // 禁止「命中即清理遗留脚本」——/run 失败时任务留守、/tr 仍指向该脚本，窗口内 unlink 会让
  // 兜底触发时 node 找不到脚本（重启丢失）。旧脚本仅两条安全路径清理：①上方放行写入点的过期
  // 清理；②janitor 菜单段统一清扫。写失败按防抖失效处理（fail-open：宁可多发不可不发）。
  lastScheduleAt = nowMs;
  try {
    writeUpdateStateSafe(Object.assign({}, readUpdateStateSafe({ stateFileOverride: s2.stateFileOverride }), {
      schema: 1,
      pendingRestartAt: Date.now() + 120 * 1000,
      restartScript: scriptPath,
    }), { stateFileOverride: s2.stateFileOverride });
  } catch (e) {
    logT('[enhance] debounce record write failed (fail-open): ' + String(e && e.message || e));
  }
  if (run.status !== 0) {
...
harness.handle('update/portRestart', async (args) => {
  // 2026-09-12（D1）：serviceName 上提到 try 之外——下方 catch 分支同样要带 err 日志尾部。
  const serviceName = args && typeof args.serviceName === 'string' && /^[A-Za-z0-9_-]+$/.test(args.serviceName) ? args.serviceName : 'dsh-web';
  try {
    // v4.12 批次A（P0-1）：自动链闸门——只拦 client 自愈链的 auto:true 调用，手动操作豁免。
    // ① kill-switch（env DSH_PE_AUTO_UPDATE=0 ∥ config update.autoUpdate=false）→ AUTO_DISABLED；
    // ② 退避闸（update-state nextRetryAt 未到 / 当日达上限）→ BACKOFF_WAITING（message 带剩余秒数）。
    const gate = autoGateDecision(args && args.auto === true);
    if (gate) return withDiagLog({ ok: false, code: gate.code, message: gate.message }, serviceName);
    ...
    if (staged) {
      ...
      logT('[enhance] update/portRestart staged install ok: ' + path.basename(staged) + ' ' + ins.message);
    }
    ...
    if (fs.existsSync(idxPath)) {
      ...
      if (!idxTrusted) logT('[enhance] portRestart: 进程索引 kind=' + (idx && idx.kind) + ' ≠ 当前 ' + runtimeKind + '（双实例覆盖），回退当前进程参数');
    } else {
      logT('[enhance] portRestart: 进程索引缺失，回退当前进程参数');
    }
    ...
    const restartMode = pickRestartMode({ isDesktop, serviceExists: serviceExistsHint(svcInfo), startTypeNum });
      ...
      if (restartMode === 'service') {
        ...
        if (sched && sched.ok === false) {
          ...
          return withDiagLog({ ok: false, code: 'SCHEDULE_FAILED', message: sched.message }, serviceName);
        }
        return {
          ok: true,
    ...
    return withDiagLog({ ok: false, code: 'PORT_RESTART_EXCEPTION', message: String(e && e.message ? e.message : e) }, serviceName);
  }
});
...
module.exports = {
  ...
  buildWebMenuCmdBody,
  // v4.13 批次B（P0-2）：自愈守卫探针脚本生成器导出——B-5 漂移锚单测与 makeShortcut 落盘用
  buildHealGuardScript,
  // 端口重启模式判定纯函数（v4.7：DISABLED 服务自动降级前台，与 v4.6 阶梯同口径）
  pickRestartMode,
  // v4.12 批次A：自动链闸门/防抖内部导出——单测注入复用同一实现，杜绝逻辑分叉
  scheduleServiceRestart,
  readAutoUpdateKillSwitch,
  autoGateDecision,
  readUpdateStateSafe,
  writeUpdateStateSafe,
  RESTART_DEBOUNCE_MS,
