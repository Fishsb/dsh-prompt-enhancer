 ...
 * updater-host.cjs 静态子命令，本壳只有 choice 分流 + 按需自提权）。用户规则 2026-08-22：
 * 一个桌面图标、[1] 启动 Web / [2] 维护菜单 两选项、choice /t 3 /d 1 三秒倒计时默认启动。
 * 权限设计：快捷方式本身**不带** RunAs（日常双击零 UAC）；[1] 冷启路径全程无需管理员；
 * 选 [2] 时 net session 探测——已管理员直接进，否则 Start-Process -Verb RunAs 自重启提权
 * （UAC 取消则降级普通权限继续，菜单内仍有逐项失败标注兜底）。
 */
function buildWebMenuCmdBody(opts) {
  ...
  return '@echo off\r\n' +
    ...
    'if /i "%~1"=="maintain" goto maintainRun\r\n' +
    'echo === DSH Web ===\r\n' +
    'echo   [1] 启动 Web（确保 3080 在线并打开网页）\r\n' +
    'echo   [2] 维护菜单（重启/更新/端口修复/体检/救援·需管理员）\r\n' +
    'choice /c 12 /t 3 /d 1 /n /m "选择 [1/2]，3 秒后默认启动 [1]: "\r\n' +
    'if errorlevel 2 goto maintainAsk\r\n' +
    upCmd + '\r\n' +
    'pause\r\n' +
    'goto :eof\r\n' +
    ':maintainAsk\r\n' +
    'net session >nul 2>&1\r\n' +
    'if %errorlevel%==0 goto maintainRun\r\n' +
    'echo [2] 需要管理员权限：正在弹出 UAC 确认（点否/取消则以普通权限继续）...\r\n' +
    'powershell -NoProfile -Command "try{Start-Process -FilePath \'%~f0\' -ArgumentList \'maintain\' -Verb RunAs|Out-Null;exit 0}catch{exit 1}"\r\n' +
    'if %errorlevel%==0 goto :eof\r\n' +
    'echo 未提权——以普通权限继续（需要管理员的项目会失败，其余可用）。\r\n' +
    ':maintainRun\r\n' +
    'title DSH Web 维护\r\n' +
    maintainCmd + '\r\n' +
    'pause\r\n';
