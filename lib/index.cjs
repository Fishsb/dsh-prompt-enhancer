...
harness.handle('update/portRestart', async (args) => {
  try {
    const serviceName = args && typeof args.serviceName === 'string' && /^[A-Za-z0-9_-]+$/.test(args.serviceName) ? args.serviceName : 'dsh-web';
    // v3.2.1-q（用户实测·安装未生效）：重启前安装 staging 待装 tarball（若有）——
    // 一键更新（staged）→ 端口重启（安装+重启）链路补齐；安装失败阻断并返回明确错误
...
function installStagedTarball(tgzPath, profile) {
  try {
    ...
    if (!bin) return { ok: false, message: 'dsh bin 未找到（resolveDshBin 失败）' };
    const r = spawnSync(bin, ['plugin', '--profile', profile, 'add', tgzPath], { encoding: 'utf8', windowsHide: true, timeout: 120000 });
    const out = String(r.stdout || '') + String(r.stderr || '');
    if (r.status !== 0 || /error|failed|✗/i.test(out)) {
      return { ok: false, message: (out.trim() || ('exit ' + r.status)).slice(0, 300) };
    }
    try { fs.unlinkSync(tgzPath); } catch (e) { /* 已安装，删除失败不阻断 */ }
    return { ok: true, message: out.trim().slice(0, 200) };
  } catch (e) {
