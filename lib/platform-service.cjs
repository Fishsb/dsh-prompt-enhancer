...
/** systemctl show MainPID 输出 → PID | null（0 = 未运行） */
function parseSystemdMainPid(stdout) {
  const m = /MainPID=(\d+)/.exec(String(stdout || ''));
  const pid = m ? Number(m[1]) : 0;
  return pid > 0 ? pid : null;
}
/** launchctl list 输出 → { exists, running, pid }（行: label PID status；PID 列 '-' = 未运行） */
function parseLaunchctlList(stdout, status) {
  const exists = status === 0;
  const m = /^(\S+)\s+(\d+|-)\s+(\d+)/m.exec(String(stdout || ''));
  const pid = m ? Number(m[2]) : 0;
  return { exists, running: exists && m !== null && m[2] !== '-', pid: pid > 0 ? pid : null };
}
...
const linux = {
  ...
  readPort(svc, env) {
    const r = probe('systemctl', ['show', svc, '-p', 'ExecStart'], env);
    if (r.ok) {
  ...
  pid(svc, env) {
    const r = probe('systemctl', ['show', svc, '-p', 'MainPID'], env);
...
const darwin = {
  ...
  startService(svc, env) {
    ...
    for (const p of plistCandidates(svc)) {
      ...
      const r = probe('launchctl', ['bootstrap', 'gui/' + uid, p], env);
      if (r.ok) return { ok: true, detail: 'bootstrapped ' + p };
    }
    return { ok: false, code: 'LAUNCHCTL_START_FAIL', message: 'launchctl start/bootstrap 均失败：请手动重启（launchctl kickstart gui/' + uid + '/' + svc + '）' };
  },
  isStopped(svc, env) {
    const r = probe('launchctl', ['list', svc], env);
    const info = parseLaunchctlList(r.stdout, r.status);
...
module.exports = {
  backendFor, win, linux, darwin, probe, plistCandidates, __setProbe,
  // 纯解析函数（单测直用，无命令依赖）
