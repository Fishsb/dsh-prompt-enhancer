...
async function workerCall(method, args) {
  const r = await httpsRequest('http://127.0.0.1:' + ASR_WORKER_PORT + '/rpc', {
...
/** 本地引擎识别：data URL → worker transcribe */
async function transcribeLocal(audioBase64) {
  try {
    const r = await workerCall('transcribe', { audioBase64 });
    if (!r || r.ok !== true) {
...
async function localStatus() {
  ...
  try {
    const r = await httpsRequest('http://127.0.0.1:' + ASR_WORKER_PORT + '/health', { method: 'GET', timeoutMs: 2000 });
...
async function transcribe(cfg, audioBase64, engineOverride) {
  ...
  if (engine === 'local') {
    const lr = await transcribeLocal(audioBase64);
    if (!lr.ok) return lr;
    const raw = lr.text;
