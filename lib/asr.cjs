...
function sanitizeVoiceCfg(cfg) {
  ...
  return {
    asr: {
      ...
      local: {
        // 模型 id（单模型 sense-voice；旧值 sensevoice-q8 兼容映射；多模型时扩展白名单）
        model: local.model === 'sense-voice' ? 'sense-voice' : 'sense-voice',
    ...
    refine: {
      ...
      // v3.2.8（用户需求·复用模型配置）：mode='chain' 走基座 llm 服务（用 enhance 模型链，
      // 免填 key）；mode='custom' 用下方独立配置（高级）。modelRef = 链中序号（0=第一条）。
      mode: rf.mode === 'custom' ? 'custom' : 'chain',
      modelRef: Number.isInteger(rf.modelRef) && rf.modelRef >= 0 && rf.modelRef < 8 ? rf.modelRef : 0,
...
/** 本地引擎状态探测（installed/modelReady/workerUp） */
async function localStatus() {
  ...
  return { installed: workerUp, modelReady, workerUp };
}
...
// 基座 llm 服务（v3.2.8：refine chain 模式复用 enhance 模型链，免填 key）——由 lib/index.cjs 注入
let llmService = null;
function setLlm(llm) {
  llmService = llm && typeof llm.stream === 'function' ? llm : null;
}

/** refine 规整：mode='chain' 走基座 llm（用 enhance 模型链），mode='custom' 走独立 OpenAI 兼容；
 * 小 maxTokens；失败返回 {ok:false, code}（不抛，降级 raw） */
async function refineText(voice, text, enhanceChain) {
  const rf = voice.refine;
  if (!rf.enabled) return { skipped: true };
  if (rf.mode === 'chain') {
    // 从 enhance 模型链取 provider/model（modelRef 越界 → 第一条；空链 → 跳过）
    const chain = Array.isArray(enhanceChain) && enhanceChain.length > 0 ? enhanceChain : null;
    if (!chain) return { skipped: true };
    const entry = chain[Math.min(rf.modelRef, chain.length - 1)] || chain[0];
    if (!entry || typeof entry.provider !== 'string' || typeof entry.model !== 'string') return { skipped: true };
...
async function status(cfg) {
  ...
  // refine.configured：chain 模式（复用模型链）→ 链非空即就绪；custom 模式 → 独立三项完整
  const chain = enhanceChainOf(cfg);
...
async function transcribe(cfg, audioBase64, engineOverride) {
  ...
  if (engine === 'local') {
    ...
    let refined = null;
    let warn = null;
    const rf = await refineText(voice, raw, enhanceChainOf(cfg));
...
// 提取 enhance 模型链（cfg.enhancer.fallback 或 cfg.fallback）——refine chain 模式用
function enhanceChainOf(cfg) {
  if (!cfg || typeof cfg !== 'object') return null;
  if (cfg.enhancer && Array.isArray(cfg.enhancer.fallback)) return cfg.enhancer.fallback;
  if (Array.isArray(cfg.fallback)) return cfg.fallback;
  return null;
}
