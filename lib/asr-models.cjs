  ...
  try {
    ...
    if (detectSessionId() <= 0) {
      ...
      try {
        ...
        // host 立即删 ps1 会致任务找不到脚本（窗口打不开）。ps1 脚本末尾自删除 +
        // host 延迟 6s 兜底删（双保险）。
        // 引号教训（2026-08-20 四次踩坑）：tr 不能显式加外层引号——node 序列化会变成
        // 双层引号嵌套，schtasks 存储的命令损坏 → 任务启动失败（ps1 不执行）。
        // 不带外层引号，node 自动引用（含空格参数），schtasks 去引号还原干净命令行。
        const tr = 'powershell.exe -NoProfile -ExecutionPolicy Bypass -WindowStyle Hidden -File ' + psFile;
        ...
        // 兜底延迟删（ps1 通常已被脚本自身删除；任务失败时 6s 后清理）
        if (wrote) {
          setTimeout(() => { try { fs.unlinkSync(psFile); } catch (e) { /* ignore */ } }, 6000);
        }
      }
