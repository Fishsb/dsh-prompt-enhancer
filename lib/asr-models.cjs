...
function triageDownloadError(msg) {
  ...
  if (/ECONNRESET|reset|aborted|socket hang up|ECONNREFUSED/i.test(s)) return '连接被重置——网络受限，可在设置配置下载代理后重试';
...
function downloadFile(url, dest, onProgress, explicitProxy, extraMirrors) {
  ...
  const tryOne = (u, redirectsLeft, byte) => new Promise((resolve, reject) => {
    ...
    try {
      ...
      // v4.9（A2）：explicitProxy 显式配置存在时强制走代理（失败仍按 proxyBroken 降级重试）
      const useProxied = explicitProxy ? !proxyBroken : (!proxyBroken && !!netProxy.resolveProxy(u));
      if (useProxied) {
        req = armTimeout(netProxy.httpsGetProxied(u, headers, handle, null, explicitProxy || undefined));
        req.on('error', (e) => { if (!explicitProxy) proxyBroken = true; reject(e); });
      } else {
