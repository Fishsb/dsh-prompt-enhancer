...
function detectSessionId() {
  ...
  try {
    const { spawnSync } = require('node:child_process');
    const r = spawnSync('powershell.exe', ['-NoProfile', '-Command', '[System.Diagnostics.Process]::GetCurrentProcess().SessionId'], { encoding: 'utf8', timeout: 5000 });
    const n = parseInt(String(r.stdout || '').trim(), 10);
    hostSessionId = Number.isFinite(n) ? n : 1;
...
function modelOpenDir() {
  ...
  try {
    ...
    if (detectSessionId() <= 0) {
      // 服务模式（Session 0）：schtasks 交互式任务在用户会话打开
      const tn = 'dsh-open-models';
      const tr = 'explorer.exe "' + dir + '"';
      spawnSync('schtasks', ['/create', '/tn', tn, '/tr', tr, '/sc', 'once', '/st', '00:00', '/it', '/f'], { timeout: 5000 });
