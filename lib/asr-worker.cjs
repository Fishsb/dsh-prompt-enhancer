...
const PORT = 3082;
const MODEL_DIR = path.join(__dirname, 'models', 'sense-voice');
...
function transcribe(dataUrl) {
  if (!recognizer || !sherpa) {
  ...
  if (req.method === 'POST' && (req.url === '/rpc' || req.url === '/rpc/')) {
    ...
    req.on('end', () => {
      ...
      try {
        ...
        else if (method === 'transcribe') result = transcribe(args.audioBase64);
...
server.listen(PORT, '127.0.0.1', () => {
  console.log('[asr-worker] listening on 127.0.0.1:' + PORT + ' | modelReady=' + !!recognizer + (loadError ? ' | ' + loadError : ''));
});
