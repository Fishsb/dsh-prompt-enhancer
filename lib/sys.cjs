...
function probeEnv(serviceName, pure, env, executorPort) {
  ...
  // 2. svc-type + svc-bin — 重启阶段直接依赖（启用状态 / 可执行文件存在）
  {
    ...
    items.push({ key: 'svc-bin', ok: binOk, warn: !binOk, detail: binDetail });
  }

  // 3. port — occupied by this very process (or free); unparseable port → warn
  //    （执行器健康检查以 netstat 判 3080 LISTENING 为 healthy——占用者非本服务
  //    进程时重启会被误判成功，此检查防误判）
  {
    let ok = true;
    let detail = 'ok';
  ...
  //    （执行器监听失败 → listen EADDRINUSE → 进程退出，一键更新直接不可用）
  {
    ...
    let ok = true;
    let detail = 'ok';
    if (portInfo.ok && portInfo.port === exPort) {
      ok = false;
      ...
      if (r.ok) {
        ...
        if (m && Number(m[1]) !== process.pid) {
          // 占用者非本服务进程（旧执行器残留由 executorEnsure 版本对齐 kill 重建，属正常）
          ok = false;
