 ...
 * pid 解析复用 dshPrimParsePortHolder 单一事实源）；posix lsof 不在探针白名单（扩面红线），
...
const PORT_PRIMS = [dshPrimNetstat, dshPrimParsePortHolder, dshPrimPortHolder, dshPrimTaskKill, dshPrimPidImage, dshPrimPidHasListening];
...
module.exports = {
  ...
  isDesktop,
  runProbe,
  // 批次C（P1-4）：probeEnv 链异步执行器 + 熔断测试缝（runProbe 保持同步形态供重启阶梯链）
  runProbeAsync,
  __setProbeExecutor,
  __resetNetBreaker,
  readServicePort,
