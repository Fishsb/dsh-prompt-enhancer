 ...
 *   apply   -> {repo, tag, profile, serviceName, restart?} — install then
 *              restart loop (restart:false = install only, service stays
 *              stopped, phase ends at 'installed' — paired with the
 *              separate `restart` RPC for a "pull & install" + "restart"
 *              two-button workflow)
...
const server = http.createServer((req, res) => {
  ...
  req.on('end', () => {
    ...
    if (method === 'apply') {
      ...
      // v2.9.x：restart:false = 只安装不重启（服务保持停止，由独立 `restart` RPC 拉起）
      const withRestart = args.restart !== false;
      ...
      (async () => {
        ...
        // 3. 记录旧版本（回滚用）
        const oldVersion = sys.readInstalledPluginVersion(profile);

        // 4. 停服 → 本地安装（staging 已就绪，不再走网络）
        state.phase = 'preparing';
        state.message = 'stopping service for install';
        const wasRunning = !serviceStopped(svc);
        const stopped = await stopService(svc);
        if (!stopped && wasRunning) {
          state.applying = false;
          state.phase = 'failed';
          state.message = 'stop service failed';
        ...
        const r = await installLocal(staged.path, profile);
        if (!r.ok) {
          state.applying = false;
          state.phase = 'failed';
          ...
          if (wasRunning) {
            log('restoring service after failed install');
            startService(svc);
          }
          return respond({ ok: false, code: r.code === 'TIMEOUT' ? 'TIMEOUT' : 'INSTALL_FAILED', message: r.message });
        }

        // 5. 安装成功 → 默认后台重启循环（不等待；client 轮询 status 获取进度/结果）；
        //    restart:false → 停服保持（服务已 stop），phase 终态 = installed，由
        //    独立的 `restart` RPC（「端口重启」按钮）拉起
        if (!withRestart) {
          state.applying = false;
          state.phase = 'installed';
          state.message = 'installed; restart manually';
          log('apply installed (restart:false) tag=' + tag + ' svc stays stopped');
          return respond({ ok: true, accepted: true, version: tag, message: 'installed' });
        }
        ...
        restartService(svc).then((rr) => {
          ...
          rollbackToVersion(svc, profile, oldVersion).then((rb) => {
            ...
            if (!rb.ok) {
              state.phase = 'failed';
              state.message = 'rollback failed: ' + rb.message;
              log('rollback FAILED: ' + rb.message);
            } else {
              state.phase = 'healthy';
              state.message = 'rolled back to ' + rb.version;
              log('rolled back to ' + rb.version);
            }
          }).catch((e) => {
            state.applying = false;
            state.phase = 'failed';
            state.message = 'rollback error: ' + String(e && e.message || e);
            log('rollback ERROR: ' + String(e && e.message || e));
          });
        }).catch((e) => {
          state.applying = false;
          state.phase = 'failed';
          state.message = 'restart error: ' + String(e && e.message || e);
          log('restart ERROR: ' + String(e && e.message || e));
        });
