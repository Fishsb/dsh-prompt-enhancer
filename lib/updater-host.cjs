...
// v3.1.6（用户指令·PID 校验）：读取服务当前 PID（sc queryex 的 PID 字段）。
// 返回 number 或 null（服务未运行/查询失败）。用于「重启成功 = PID 已更新」判定——
// 端口监听可能被旧进程残留占用，只有服务进程 PID 真正变化才证明服务重启过。
function servicePid(svc) {
  const r = sys.runProbe('sc', ['queryex', svc], env());
  if (!r.ok) return null;
  const m = /PID\s*:\s*(\d+)/i.exec(r.stdout);
  ...
  try {
    ...
    for (let attempt = 1; attempt <= plan.maxAttempts; attempt++) {
      ...
      while (Date.now() - probeStart < HEALTH_PROBE_MAX_MS) {
        ...
        if (portListening(healthPort) && newPid !== null && newPid !== oldPid) { healthy = true; break; }
      ...
      log('round ' + attempt + ' NOT ready: listening=' + portListening(healthPort) + ' pid=' + servicePid(svc) + ' oldPid=' + oldPid);
      await sleep(5000);
    }
    state.phase = 'failed';
