...
//                          [--run 1..5]        （非交互：执行单项后退出——冒烟/演练用）
// 五项职责互斥（方案 §11.1）：1=不动版本原地重启；2=装 staging 动版本；
...
/** sc query 原文状态解析（RUNNING/STOPPED/START_PENDING/…；不存在 → {exists:false}）。 */
function svcStateRaw(svc) {
  ...
  return { exists: true, state: m[1].toUpperCase() };
}
