...
async function rollbackToVersion(svc, profile, oldVersion, seam) {
  ...
  await (s2.stopService || stopService)(svc);
  ...
  (s2.startService || startService)(svc);
...
async function watchdogTick() {
  ...
  // v3.2.1-c（2026-08-19 用户实测·插件自洽接管）：服务出现（用户手动装好 nssm 服务）
...
async function restartService(svc, opts) {
  const o = opts || {};
  const say = o.onLine || (() => {});
  if (state.busy) return { ok: false, code: 'BUSY', message: 'restart already in progress' };
  // 2026-09-12（审查修复·D5）：新一轮起点（busy 闸之后、一切早退之前）清空上一轮 diagLog——
  // 旧值永不清除，客户端在 phase='restarting' 轮询时会把上一轮失败的根因当本轮根因渲染（误归因）。
  // 放在早退之前：NOT_ADMIN 等早退同样是「新一轮」，旧根因此刻已经失效。失败路径会重新写入。
  state.diagLog = undefined;
  // v4.11（安全门·必须先于一切副作用）：显式声明非管理员 → 立即早失败。
  // 教训（2026-08-24）：本函数在「服务不存在」时会读真实进程索引做进程级重启——
  // 单测传假服务名也会杀到真身，故 adminOverride:false 的早退必须放在最前面。
  if (o.adminOverride === false && process.platform === 'win32') {
    state.phase = 'failed';
    state.message = 'NOT_ADMIN';
    say('✗ 非管理员——sc stop 将被拒绝');
    say('  出路①右键快捷方式→以管理员身份运行；出路②设置页「端口重启」（SYSTEM 任务链）；出路③维护菜单5救援。');
    return { ok: false, code: 'NOT_ADMIN', message: 'not elevated — sc stop would be denied; run as administrator or use settings-page port restart' };
  }
  state.busy = true;
  ...
  // v4.11（2026-08-24 实测·STOP_FAILED 排障）：补充端口实际监听者——服务 PID 与监听者不一致
  // 说明存在双实例（前台孤儿 vs 服务节点），健康判定必须以端口为准
  const listenerPid0 = sys.portHolderPid(healthPort);
  log('restart start svc=' + svc + ' healthPort=' + healthPort + ' oldPid=' + (oldPid === null ? 'none' : oldPid)
    + ' listenerPid=' + (listenerPid0 === null ? 'none' : listenerPid0)
    + (oldPid !== null && listenerPid0 !== null && oldPid !== listenerPid0 ? ' (MISMATCH: possible orphan foreground instance)' : ''));
  ...
  // v4.11（2026-08-24 实测）：非提权控制台 sc stop 必被拒绝（Access denied）——此前盲等 20s
  // 才报 STOP_FAILED 且无指引。先预检管理员，直接早失败并给出可执行出路。
  const admin11 = o.adminOverride !== undefined ? !!o.adminOverride : mIsAdmin();
  if (process.platform === 'win32' && !admin11) {
    state.phase = 'failed';
    state.message = 'NOT_ADMIN';
    say('✗ 当前会话非管理员——sc stop 将被拒绝，跳过 20s 盲等');
    say('  出路①右键快捷方式→以管理员身份运行；出路②设置页「端口重启」（SYSTEM 任务链）；出路③维护菜单5救援。');
    state.busy = false;
    return { ok: false, code: 'NOT_ADMIN', message: 'not elevated — sc stop would be denied; run as administrator or use settings-page port restart' };
  }
  let escalated11 = false;
  try {
    say('停止服务 ' + svc + '…');
    ...
    // v4.11：20s 停不下先升级强停一轮（P2 同款语义），再判失败——杜绝「只报错不给出路」
    if (!stopped) {
      escalated11 = true;
      say('⚠ 20s 未 STOPPED——升级强制停止（taskkill /T 服务进程树）…');
      if (o.forceStopImpl) { await o.forceStopImpl(svc); }
      else {
        const spid = backend.pid ? backend.pid(svc, env()) : null;
        const target = spid || oldPid || listenerPid0;
        if (target) killPidTree(target);
      }
      for (let i = 0; i < 10; i++) {
        if (serviceStopped(svc)) { stopped = true; break; }
        await sleep(1000);
      }
      if (stopped) say('✓ 强制停止生效');
    }
    ...
    for (let attempt = 1; attempt <= plan.maxAttempts; attempt++) {
      ...
      if (!roundStopped) {
        // v4.11：轮内未停 → 升级强停一次后重查；仍不停才失败（权限已在入口预检）
        if (!escalated11) {
          escalated11 = true;
          say('⚠ 轮内 round ' + attempt + ' 未 STOPPED——升级强制停止…');
          if (o.forceStopImpl) { await o.forceStopImpl(svc); }
          else {
            const spid = backend.pid ? backend.pid(svc, env()) : null;
            const target = spid || oldPid || listenerPid0;
    ...
    // 2026-09-08 产品改进（90s 超时根因直显）：重启链穷尽失败 = DSH 本体大概率起不来
    // （boot 崩溃循环，实测案例 dsh-shoucang-memory JsonSchemaError）——此刻执行器是
    // 唯一存活的诊断者，抓 dsh-web err 日志尾部随 status 下发，client 直显根因。
    state.diagLog = dshErrLogTail(svc);
...
// 读 DSH web err 日志尾部：服务模式经 nssm 注册表 AppStderr 定位（REG_SZ/REG_EXPAND_SZ
// 均兼容）；查不到（非 nssm/无服务/桌面）降级执行器前台重启 err 日志；再无则空串，
// client 对空串不渲染诊断块（降级静默，不影响原有错误文案）。
// 2026-09-12（审查修复·D3）：取尾部 N 行之前先做「降噪 + 优先根因」——
// 旧版直接 slice(-n)，真机日志尾部被 Node ExperimentalWarning 刷满 → 8 行窗口全是噪声、
// 真根因（Error: dsh: cannot resolve profile bundle …）被挤在窗口外，UI 只看到警告。
// 契约不变：仍返回字符串（无内容时空串），仍受 maxLines 约束。
  ...
  req.on('end', () => {
    ...
    if (method === 'restart') {
      ...
      (async () => {
        state.applying = true;
        state.phase = 'restarting';
        // 2026-09-12（审查修复·D5）：同 restartService——HTTP 入口也清空陈旧 diagLog，
        // 否则 status 在新一轮 restarting 阶段回吐上一轮根因（客户端据此误归因）。
        state.diagLog = undefined;
        state.message = 'stopping';
...
async function restartCore(svc, profile, onLine) {
  ...
  const result = await restartService(svc, { onLine: say });
...
//                          [--run 1|2|3|4|5]   （非交互：执行单项后退出——冒烟/演练用）
// 五功能全自动（v4.7 收敛·v4.9 全程计时提醒·批次A 增 4=陈旧任务清扫·批次B 增 5=一致性自检）：
// 1=一键拉起（ensureWebUp 阶梯）；2=端口修复（清占用者后拉起）；
// 3=一键更新（staging 安装→闸门→重启生效→G11）；4=sweepStaleExecTasks（A.1.6-c janitor）；
// 5=runInstallConsistencyAudit（P1-3 只读五项）。
...
function confirmNssmUninstall(io, svc, reason, seam) {
  ...
  io.out('  可逆性说明：如需服务化可手动重装 nssm 服务（插件已不提供安装入口）；本次及历史处置均留痕 web-port-recovery.log');
...
async function degradeNssmService(io, svc, reason, seam) {
  ...
  if (!proceed) {
    ...
    io.out('✗ 已保留 nssm 服务（未卸载）——继续前台冷启兜底；服务可稍后手动处置（插件已不提供安装入口）');
  ...
  try {
    ...
    io.out('✓ 服务已卸载（如需可手动重装 nssm 服务，插件已不提供安装入口）；已留痕 web-port-recovery.log');
    return { ok: true };
  } catch (e) {
...
// 2026-09-12（审查修复·死代码）：原为空占位函数，调用方 killConflictingHolders 的击杀动作
// 因此零留痕（调用方 summary 只在 io.out 打印，非交互式 schtasks 拉起时无人看到 stdout）。
// 现按同区既有留痕纪律落 web-port-recovery.log —— 字段/时间戳格式与 degradeNssmService 一致，
// 失败静默不阻断（留痕尽力，绝不因日志写失败影响端口恢复主链）。签名不变。
function tracePush(line) {
  try {
    fs.appendFileSync(path.join(sys.EXECUTOR_ROOT, 'web-port-recovery.log'),
      '[' + new Date().toISOString() + '] ' + String(line) + '\n', 'utf8');
  } catch { /* 留痕尽力：目录不可写/磁盘满等一律不阻断 */ }
}
...
async function repairWebPort(io, svc, profile, opts) {
  const o = opts || {};
  io.out('[2] 端口修复：将清理占用者并重建 Web（服务接管时＝强制重启服务树）…');
...
/**
 * v4.12 批次A（A.1.6-c ·janitor 收编维护菜单）：扫描 dsh-prompt-enhancer-exec-* 陈旧
 * 计划任务（创建时间 >24h，时间戳取自任务名尾段——生成时 Date.now()，免 locale 解析），
 * 逐条 /query /tn X /xml 导出备份并核验非空后 /delete /f，最后全表复查打印清单。
 * 完全沿用 2026-08-24 手工清理规程，只是固化成代码。opts 注入缝供单测隔离。
 */
async function sweepStaleExecTasks(io, opts) {
  const o = opts || {};
  const STALE_MS = 24 * 60 * 60 * 1000;
  const systemRoot = process.env.SystemRoot || process.env.windir || 'C:/Windows';
  const schtasksBin = o.schtasksBin || path.join(systemRoot, 'System32', 'schtasks.exe');
  const query = o.queryImpl || ((args) => spawnSync(schtasksBin, args, { windowsHide: true, encoding: 'utf8', timeout: 20000 }));
  const now = Date.now();
  const q = query(['/Query', '/FO', 'CSV', '/NH']);
  if (!q || q.status !== 0) {
    io.out('⚠ 计划任务全表查询失败，跳过清扫');
    return { ok: false, scanned: 0, cleaned: [], remaining: -1 };
  }
  const names = String(q.stdout || '').split(/\r?\n/)
    .map((l) => { const m = /^"([^"]+)"/.exec(l.trim()); return m ? m[1] : ''; })
    .filter((n) => /^dsh-prompt-enhancer-exec-/i.test(n));
  io.out('发现 exec-* 任务 ' + names.length + ' 条，逐条核对创建时间（>24h 才清）…');
  const cleaned = [];
  for (const name of names) {
    const m = /^dsh-prompt-enhancer-exec-\d+-(\d+)$/i.exec(name);
    const ts = m ? Number(m[1]) : 0;
    if (!ts || !Number.isFinite(ts) || (now - ts) < STALE_MS) {
      io.out('  保留 ' + name + '（创建不足 24h 或名称不含时间戳）');
      continue;
    }
    // 纪律（破坏性操作前必须可验证备份）：/xml 导出核验非空才删
    let backupFile = '';
    try {
      const x = query(['/Query', '/TN', name, '/XML']);
      if (x && x.status === 0 && String(x.stdout || '').trim().length > 20) {
        const backupDir = o.backupDir || path.join(sys.dshHomeDir(), 'backups',
          'schtasks-orphan-' + new Date(now).toISOString().replace(/[-:]/g, '').replace(/\..+$/, '').replace('T', '-'));
        fs.mkdirSync(backupDir, { recursive: true });
        backupFile = path.join(backupDir, name + '.xml');
        fs.writeFileSync(backupFile, String(x.stdout), 'utf8');
      }
    } catch { /* 备份异常按不可得处理 */ }
    if (!backupFile) {
      io.out('  ⚠ 跳过 ' + name + '（XML 备份不可得——宁留勿删）');
      continue;
    }
    const d = query(['/Delete', '/TN', name, '/F']);
    if (d && d.status === 0) {
      io.out('  ✓ 已清理 ' + name + '（备份: ' + backupFile + '）');
      cleaned.push(name);
    } else {
      io.out('  ⚠ 删除失败 ' + name + ' exit=' + (d ? d.status : '?') + ' ' + String((d && d.stderr) || '').trim().slice(0, 120));
    }
  }
  const re = query(['/Query', '/FO', 'CSV', '/NH']);
  const left = re && re.status === 0
    ? String(re.stdout || '').split(/\r?\n/).filter((l) => /dsh-prompt-enhancer-exec-/i.test(l)).length
    : -1;
  io.out(left >= 0
    ? ('复查完成：本次清理 ' + cleaned.length + ' 条，残留 exec-* ' + left + ' 条')
    : '⚠ 复查查询失败（删除结果见上方逐条输出）');
  return { ok: true, scanned: names.length, cleaned, remaining: left };
}
...
async function runMaintainMenu(opts) {
  ...
  const dispatch = async (choice) => {
    ...
    else if (choice === '4') await runWithTiming(io, '陈旧任务清扫', (tio) => sweepStaleExecTasks(tio, {}));
    else if (choice === '5') await runWithTiming(io, '安装一致性自检', (tio) => runInstallConsistencyAudit(tio, profile));
  ...
  for (;;) {
    ...
    io.out('  4) 任务清扫   —— 清理创建超 24h 的 exec-* 残留计划任务（先 XML 备份后删除，附复查清单）');
    io.out('  5) 一致性自检 —— 只读五项安装一致性报告（缓存 tgz/账本/file: 依赖/版本基线）');
    io.out('  0) 退出');
...
module.exports = {
  ...
  hashGate,
  fetchExpectedSha256,
  // v4.12 批次A：熔断/防抖与 janitor 导出——单测注入复用同一实现
  sweepStaleExecTasks,
  readUpdateState,
  writeUpdateState,
  bumpUpdateFail,
  ...
  watchdogStop,
  watchdogTick,
  // 2026-09-08 产品改进：err 日志尾诊断导出——单测注入复用同一实现
  dshErrLogTail,
  redactDiagLine,
  WATCH_INTERVAL_MS,
