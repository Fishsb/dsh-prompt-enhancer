...
async function restartService(svc) {
  ...
  if (!backend) {
    state.phase = 'failed';
    state.message = 'unsupported platform: 当前平台不支持自动服务重启，请手动重启 DSH';
    log('UNSUPPORTED PLATFORM for service restart');
    return { ok: false, code: 'UNSUPPORTED_PLATFORM', message: 'platform does not support automatic service restart' };
  }
  try {
