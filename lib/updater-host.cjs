 ...
 *   apply   -> {repo, tag, profile, serviceName, port} — install then restart loop
 *   restart -> {serviceName, port} — restart loop only
 */
...
async function restartService(svc, port) {
  ...
  log('restart start svc=' + svc + ' port=' + port);
  try {
    ...
    const plan = pure.buildRestartPlan(svc, port, 5);
...
const server = http.createServer((req, res) => {
  ...
  req.on('end', () => {
    ...
    if (method === 'restart') {
      ...
      const port = Number.isInteger(args.port) && args.port > 0 ? args.port : 3080;
      restartService(svc, port).then(respond);
      return;
    }
    if (method === 'apply') {
      ...
      (async () => {
        ...
        log('apply start tag=' + tag + ' profile=' + profile + ' svc=' + svc + ' port=' + port);
