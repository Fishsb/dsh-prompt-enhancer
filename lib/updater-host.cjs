...
/** up 动词互斥锁（R3：双击两次=双前台实例抢 3080+索引双写）。pid 探活+陈旧接管，复用 rescue.lock 模式。 */
function acquireUpLock(lockPathOverride) {
  const p = lockPathOverride || upLockPath();
  try {
    if (fs.existsSync(p)) {
      let info = null;
      try { info = JSON.parse(fs.readFileSync(p, 'utf8')); } catch { /* 损坏锁按陈旧处理 */ }
      ...
      if (info && Number.isInteger(info.pid)) {
        if (info.pid === process.pid) alive = true; // 同进程重入同样视为占用（ensureWebUp 非可重入）
        else {
          try { process.kill(info.pid, 0); alive = true; } catch (e) { alive = e && e.code === 'EPERM'; }
        }
      }
      const fresh = info && Number.isInteger(info.ts) && (Date.now() - info.ts < UP_LOCK_STALE_MS);
      if (alive && fresh) return { ok: false, message: '另一个「一键拉起」正在执行（pid=' + info.pid + '）——请等它结束。' };
    }
    fs.mkdirSync(path.dirname(p), { recursive: true });
    fs.writeFileSync(p, JSON.stringify({ pid: process.pid, ts: Date.now() }), 'utf8');
    return { ok: true };
...
function releaseUpLock(lockPathOverride) {
  const p = lockPathOverride || upLockPath();
  try { if (fs.existsSync(p)) fs.unlinkSync(p); } catch { /* 尽力而为 */ }
}
...
async function ensureWebUp(io, svc, profile, opts) {
  ...
  try {
    ...
    const startForeground = async (cmd2) => {
      ...
      const upOk = o.waitFgImpl ? await o.waitFgImpl() : await waitWebReady(io, 45000, webPort);
      if (upOk) {
    ...
    const runRecoveryLadder = async () => {
      ...
      const waitReady = (queueKey, ms) => (
        o[queueKey] ? o[queueKey]() : waitWebReady(io, ms, webPort)
      );
      // P1 软重启 ≤20s
      io.out('P1 软重启服务…');
      trace.push('P1.stop');
      ...
      await startOnce();
      if (await waitReady('waitServiceImpl', 15000)) return true;
      ...
      if (await waitReady('waitServiceImpl2', 15000)) return true;
      // P3 末次一发 ≤28s
      io.out('P3 末次重启尝试…');
      st = o.svcInfoOverride || svcStateRaw(svc);
      ...
      await startOnce();
      if (await waitReady('waitServiceImpl3', 15000)) return true;
      return false;
    };
    ...
    if (st.exists && st.state === 'RUNNING') {
      ...
      if (st.startType === 'DISABLED') {
        ...
        let ready = o.waitServiceImpl ? await o.waitServiceImpl() : await waitWebReady(io, 15000, webPort);
        if (!ready) {
          io.out('宽限二次探测（3s 后再探一次）…');
          ready = o.graceProbeImpl ? await o.graceProbeImpl() : await (sleep(3000).then(() => webReadyOnce(webPort)));
        }
        if (ready) {
...
/**
 * [2] 端口修复（全自动兜底）：清占用者 → ensureWebUp 恢复阶梯。零提问。
...
async function runMaintainMenu(opts) {
  ...
  const dispatch = async (choice) => {
    ...
    if (choice === '1') await ensureWebUp(io, svc, profile, {});
    else if (choice === '2') await repairWebPort(io, svc, profile, {});
    else if (choice === '3') await runOneClickUpdate(io, profile, {});
  ...
  for (;;) {
    printMaintainHeader(io, svc, profile);
    io.out('  1) 启动 Web   —— 拉起端口并保持在线（关机重启后同样适用；nssm 异常自动自愈）');
...
if (require.main === module) {
  if (argValue('cli') === 'restart') {
    ...
    repairWebPort(ioRp, argValue('service') || 'dsh-web', argValue('profile') || 'web', {})
      .then((r) => process.exit(r.ok ? 0 : 2))
      .catch((e) => { console.error('端口修复异常: ' + (e && e.stack || e)); process.exit(1); });
  } else if (argValue('cli') === 'update') {
    const ioUd = defaultIo();
    runOneClickUpdate(ioUd, argValue('profile') || 'web', {})
      .then((r) => process.exit(r.ok ? 0 : 2))
