...
// 2026-08-18（进程级重启降级·参考 dsh-restart）：host 在 DSH 进程内启动时写进程索引，
// 供更新执行器在「非服务化部署」（无系统服务）时进程级重启——kill 旧进程 → spawn 同参数
// 新进程 → 端口接管。文件：$DSH_HOME/dsh-prompt-enhancer.json（尽力而为，失败不影响插件）。
{
  try {
    const dshHome = process.env.DSH_HOME || String(process.env.HOME || process.env.USERPROFILE || '') + '/.dsh';
    const fsMod = require('node:fs');
    fsMod.mkdirSync(dshHome, { recursive: true });
    fsMod.writeFileSync(dshHome + '/dsh-prompt-enhancer.json', JSON.stringify({
      pid: process.pid,
...
const SYSTEM_PUBLISH_PROMPT = [
  ...
  '',
  '【需求表述纪律】（IEEE 29148 / INCOSE 需求写法，适用于所有章节）',
  ...
  apply(ctx) {
    ...
    async function enhanceStageAnalyze(state) {
      ...
      const BUILTIN_TEMPLATES = {
        // v3.1.8（用户指令·每模式默认模板按场景定制）：base 用通用语义重构模板；
        // lite/standard/smart 用各自专属默认模板（上轮参考 / 多轮脉络 / 项目事实优先）；
        // v3.2.x（2026-08-18）：T2 增量模板每模式专属（base 通用 / lite 上轮延续 / standard 多轮演进 /
        // smart 开发向 / publish 九章规格），取代原共享 supplement/dev。
        base: [SYSTEM_PROMPT, SYSTEM_INCREMENT_PROMPT],
        lite: [SYSTEM_LITE_PROMPT, SYSTEM_INCREMENT_LITE_PROMPT],
        standard: [SYSTEM_STANDARD_PROMPT, SYSTEM_INCREMENT_STANDARD_PROMPT],
        smart: [SYSTEM_SMART_PROMPT, SYSTEM_INCREMENT_SMART_PROMPT],
