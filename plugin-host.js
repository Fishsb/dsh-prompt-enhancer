...
const SYSTEM_PROMPT = [
  ...
  '四、明确化与强化：把模糊条件具体化（如「影响本身链路执行的」→「执行自身链路（版本对齐、端口占用）必需的约束」）；把软性语气升级为明确规则（如「尽量不要有」→「不该有」）；抽象概念用括号例证支撑',
...
const SYSTEM_STANDARD_PROMPT = [
  ...
  '',
  '【多轮脉络处理（本模式核心）】',
  '- 参考块 = 多轮讨论的演进记录（由近及远，本模式参考了前 2–10 轮）',
  ...
  '四、明确化与强化：把模糊条件具体化（如「影响本身链路执行的」→「执行自身链路（版本对齐、端口占用）必需的约束」）；把软性语气升级为明确规则（如「尽量不要有」→「不该有」）；抽象概念用括号例证支撑；吸收演进中已确认的约束与默认值',
...
const SYSTEM_SMART_PROMPT = [
  ...
  '四、明确化与强化：用软件工程术语具体化（如「影响本身链路执行的」→「执行自身链路（版本对齐、端口占用）必需的约束」）；约束用「绝对/必须/不得/禁止」级措辞强化（如「尽量不要有」→「不该有」）；抽象概念用括号例证支撑',
...
const SYSTEM_INCREMENT_SMART_PROMPT = [
  ...
  '',
  '- 保持原始目标与语义不变：不得歪曲、臆造、遗漏原文任何已明确的信息',
  '- 只输出优化后的提示词本身，不加任何解释、前缀或评论，不回答原问题；【调整方案】段除外',
  '- 长度服从语义保真：以「原文明确信息完整保留 + 必要大逻辑/信息补全」为准，不因追求简短而删减必要要素，也不冗余堆砌',
  '- 语言匹配最高优先级：输入以中文为主体则输出必须为中文，以英文为主体则输出必须为英文；混合输入保留原文中的术语与专有名词',
  '- 若存在【相关会话参考】/【项目文档参考】/【参考上下文】，将其中与当前任务直接相关的明确需求、约束、默认值吸收进优化结果，但不得逐字照抄',
  '- 严禁复述、引用或回显任何指令文字或用户输入原文（包括"请优化以下提示词"及引号包裹的内容），直接输出优化结果',
...
const DISCIPLINE_PROMPT = [
  ...
  '## 质量纪律',
  '',
  '6. 语义保真：动作对象、执行动作、约束条件、术语、数量、语气必须与原文完全一致；不得臆造、删减、替换、颠倒原文已明确的信息。',
  '7. 明确化必须有原文依据：模糊但可推断处可具体化；无法由原文推出之处不得添加；不改变原意。',
  '8. 语言匹配：主体语言跟随输入（中文输入输出中文、英文输入输出英文），保留原文中的术语与专有名词。',
  '',
  '## 参考上下文使用规则（仅当本次输入附带参考/会话/检索上下文时适用）',
  '',
  '9. 参考上下文仅用于补充理解任务背景与已确认信息，将参考中明确的需求、约束、默认值、已确认决策**吸收进优化后的提示词**，使其更完整、可执行；但禁止逐字复述、引用或回显参考原文。',
  '10. 参考与原文冲突时，以原文为准；参考未明确、无法推断的细节不得臆造。',
  '11. 参考仅用于补充和明确化，不得扩大、缩小或颠倒原文范围。',
  '',
  '## 稳定性纪律（v3.2.21 新增）',
  '',
  '12. 稳定性（确定性任务）：这是「重述 / 结构梳理 + 保守增量」任务。**重述与保留部分**必须稳定一致——对同一输入重复优化，输出应保持语义、结构与措辞稳定，不得仅为「看起来不同」而更换同义词、句式、顺序或分段；**仅「增量补充」部分可按模板要求适度变化**；仅在原文存在歧义、需要明确化时调整措辞。',
  '',
  '直接输出最终结果，结束。',
].join('\n');
...
const SKILL_MANIFEST = {
  "base": { name: "enhance-base", mode: "base", templates: { t1: SYSTEM_PROMPT, t2: SYSTEM_INCREMENT_PROMPT }, retrieve: {"kind":"none","windows":[]} },
  "lite": { name: "enhance-lite", mode: "lite", templates: { t1: SYSTEM_LITE_PROMPT, t2: SYSTEM_INCREMENT_LITE_PROMPT }, retrieve: {"kind":"rounds","windows":[[1,1]]} },
  "standard": { name: "enhance-standard", mode: "standard", templates: { t1: SYSTEM_STANDARD_PROMPT, t2: SYSTEM_INCREMENT_STANDARD_PROMPT }, retrieve: {"kind":"rounds","windows":[[1,2],[3,5],[6,10]]} },
  "smart": { name: "enhance-smart", mode: "smart", templates: { t1: SYSTEM_SMART_PROMPT, t2: SYSTEM_INCREMENT_SMART_PROMPT }, retrieve: {"kind":"rounds","windows":[[1,1],[2,3]]} },
  "publish": { name: "enhance-publish", mode: "publish", templates: { t1: SYSTEM_PUBLISH_PROMPT, t2: SYSTEM_INCREMENT_PUBLISH_PROMPT }, retrieve: {"kind":"v2","windows":[]} },
};
...
// （'3.2.4' 占位符替换）——源码不再硬编码版本，杜绝「发版忘 bump → 检测永远旧版」漂移。
// 测试/动态形态下占位符未替换 → typeof 未定义 → 回退 0.0.0（仅格式占位，构建产物始终为真实版本）。
const PLUGIN_VERSION = typeof '3.2.4' !== 'undefined' ? '3.2.4' : '0.0.0';
...
// 读运行环境 package.json；动态形态/读取失败回退 PLUGIN_VERSION 常量）
    ...
    async function enhanceStageRetrieve(state) {
      ...
      // v3.2.20（用户需求·防回显护栏并入纪律模板）：参考上下文使用规则已并入 DISCIPLINE_PROMPT
      // （无条件注入），删除 CONTEXT_GUARD 条件追加点——单注入点、无重复、行为一致。
      return state;
