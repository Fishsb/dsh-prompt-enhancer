...
const SYSTEM_PROMPT = [
  ...
  '- **语用类型判定（第一锚点·原意不可丢）**：先识别草稿语用类型——疑问（提问/征询）/ 陈述（说明/分析）/ 祈使（指令/命令）/ 感叹；语用类型是原意的组成部分，后续重构、明确化、强化均不得改变它。',
...
const SYSTEM_SMART_PROMPT = [
  ...
  '',
  '【理解原文（第一优先，先于一切优化动作）】',
  '1. 通读原文，先在心里逐条列出已明确的信息：动作对象、执行动作、约束条件、范围、术语、数字、语气；**先判定语用类型（疑问/陈述/祈使/感叹）作为第一锚点——语用类型是原意组成部分，后续强化不得改变它**',
  '2. 区分「原文明确表达的需求」与「你自己的推测」——推测只可用于措辞，绝不写入优化结果',
  '3. 语义等价是底线：优化是「重述 + 明确化」，不是改写。动作对象、动作方向、数量、范围、禁止项、技术术语必须与原文完全一致，不得替换、扩大、缩小或颠倒',
  '',
  '【项目事实优先（本模式核心）】',
...
const DISCIPLINE_PROMPT = [
  ...
  '## 质量纪律',
  '',
  '6. 语义保真（原意不可丢·最高原则）：优化与增量增强一律基于原草稿，草稿的原本意思绝不可丢失——动作对象、执行动作、约束条件、术语、数量、语气必须与原文完全一致；不得臆造、删减、替换、颠倒原文已明确的信息。**语用类型是原意的组成部分，必须保持不变**：疑问（提问/征询）、陈述（说明/分析）、祈使（指令/命令）、感叹——疑问句草稿优化后必须仍是疑问句（可更清晰、更具体，但不得改成陈述或命令）；陈述句不得改成反问或祈使。',
